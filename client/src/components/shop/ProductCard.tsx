...
const ProductCard = ({product, cardVariants, index}) =>{

    const navigate = useNavigate();
    return (
        <>
            <motion.div key={product.id} className="card overflow-hidden relative cursor-pointer"
            ...
            <div className="card-img bg-cover bg-top relative h-48" style={{ backgroundImage: `url(${product.image})` }}>
                <motion.div className="absolute inset-0 flex items-center justify-center" initial={{ opacity: 0, y: 20 }} 
                whileHover={{
                    opacity: 1,
                    y: 0,
